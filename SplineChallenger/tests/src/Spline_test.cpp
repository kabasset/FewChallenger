...
BOOST_AUTO_TEST_CASE(scalar_real_test) {
  const Focs::SplineIntervals u {1, 2, 3, 4};
  const std::vector<double> x {1.1, 2.5, 3.9};
  const std::vector<double> v {10, 20, 30, 40};
  const Focs::Spline<double> spline(u, v);
  std::vector<double> y;
  for (const auto& e : x) {
...
BOOST_AUTO_TEST_CASE(vector_real_test) {
  const Focs::SplineIntervals u {1, 2, 3, 4};
  const Focs::SplineArguments x(u, {1.1, 2.5, 3.9});
  const std::vector<double> v {10, 20, 30, 40};
  const Focs::Spline<double> spline(u, v);
...
BOOST_AUTO_TEST_CASE(scalar_complex_test) {
  ...
  const Focs::Spline<std::complex<double>> spline(u, v);
